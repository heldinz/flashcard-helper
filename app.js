document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadWiktionaryResults(word) {
        try {
            wiktionaryContent.innerHTML = '<p class="loading-message">Loading Wiktionary results...</p>';
            ...
            wiktionaryContent.innerHTML = '<p class="error-message">Failed to load Wiktionary content. Make sure the server is running.</p>';
        }
    }

    async function loadLangeekResults(word) {
        try {
            langeekContent.innerHTML = '<p class="loading-message">Loading Langeek results...</p>';
            ...
            if (response.ok && Array.isArray(data) && data.length > 0) {
                ...
                langeekContent.innerHTML = '<p class="error-message">No results found in Langeek</p>';
            }
        } catch (error) {
            console.error('Error fetching Langeek content:', error);
            langeekContent.innerHTML = '<p class="error-message">Failed to load Langeek content. Make sure the server is running.</p>';
        }
    }
